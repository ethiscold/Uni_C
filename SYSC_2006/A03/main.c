...
int main()
{
    ...
    */
    ...
    while (option != 3){
        ...
        scanf("%d", &option);
        // make sure the option is between 1 and 3
        ...
        if (option == 2){
            ...
            if (searchu == NULL){
                ...
                scanf("%s", password);
                printf("\n");
                if (strcmp(searchu->password, password) == 0){
                    break;
                }
                else{
        ...
        if (option == 3){
            ...
            while (choice != 5){
                ...
                // add a friend
                    ...
                    printf("Please enter a username to add: \n");
                    scanf("%s", fname);
                ...
                //display a friend's posts
                else if (choice == 4){
                    user_t *fri = users;
                    printf("Please enter a username: ");
                    scanf("%s", username);
                    printf("\n");
                    fri = find_user(users, username);
                    if (fri == NULL){
                        printf("Invalid username! \n");
                    }
                    else if (strcmp(fri->username, username)==0){
                        display_friend_posts(fri);
                    }
                }
            }
        }
        //display all posts (code in a3_functions)
        if (option == 4){
            display_all_posts(users);
        }
        // if option is 5, exit the while loop and terminate the program
    }
    // remove all users from the linked list of users to eliminate any memory drain / leaks
    teardown(users, users->posts, users->friends);
    printf("---------------------------------------\n");
